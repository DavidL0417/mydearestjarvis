...
- Canvas extension control-plane rows are public metadata with RLS, not credentials. `canvas_extension_sessions`, `canvas_extension_commands`, `canvas_extension_command_events`, and `canvas_extension_nodes` may store connection status, command state, progress events, URLs, titles, selection state, and compact evidence only.
